...
{
  int x;
  if (p) for (x = 0 ; x < p->Size; x ++)
  {
    Song *s=(Song *)calloc(1,sizeof(Song));
    if (p->Items[x].album) s->album=strdup(p->Items[x].album);
    if (p->Items[x].artist) s->artist=strdup(p->Items[x].artist);
    if (p->Items[x].title) s->title=strdup(p->Items[x].title);
    if (p->Items[x].genre) s->genre=strdup(p->Items[x].genre);
    if (p->Items[x].filename) s->filename=strdup(p->Items[x].filename);
...
void loadSongList()
{
  ...
  p->filename = strdup("http://www.firehose.net/~deadbeef/media/Misc/music/030223%20-%20pervert-in-a-satellite.mp3");
  p->album=strdup("SEP");
  p->artist=strdup("Nullsoft Band");
  p->genre=strdup("Shit");
  p->songlen = 666;
  p->track_nr=1;
  p->title=strdup("Pervert  In A Satellite");
  p->year=2003;
    ...
    case WM_PAINT:
      {
        ...
        if (wad_DrawChildWindowBorders) wad_DrawChildWindowBorders(hwndDlg,tab,2);
      }
    return 0;
    case WM_DESTROY:
      //clearSongList();
