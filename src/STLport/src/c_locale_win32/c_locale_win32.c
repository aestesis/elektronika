...
# endif
  ...
  {
    static char ansi_date_fmt[ALIB_PATH]; // Hack
    const char *cur_char;
    char *cur_output;
  ...
  const char* _Locale_d_fmt(struct _Locale_time* ltime)
  {
    ...
    GetLocaleInfoA(ltime->lcid, LOCALE_SSHORTDATE, date_fmt, ALIB_PATH);
  ...
  {
    static char date_fmt[80];
    GetLocaleInfoA(ltime->lcid, LOCALE_SLONGDATE, date_fmt, ALIB_PATH);
    __ConvertFromACP(date_fmt, 80, ltime->cp);
  ...
  {
    char time_fmt[80];
    static char ansi_time_fmt[ALIB_PATH]; // Hack
    char *cur_char, *cur_output;
    GetLocaleInfoA(ltime->lcid, LOCALE_STIMEFORMAT, time_fmt, ALIB_PATH);
