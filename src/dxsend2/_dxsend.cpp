...
// File: Grabber.cpp
//
// Desc: DirectShow sample code - Implementation file for the SampleSender
//       example filter
//
...
const AMOVIESETUP_PIN psudSampleSenderPins[] =
{ { L"Input"            // strName
  , FALSE               // bRendered
  , FALSE               // bOutput
  , FALSE               // bZero
...
const AMOVIESETUP_FILTER sudSampleSender =
{ &CLSID_SampleSender            // clsID
, L"SampleSender Example"        // strName
, MERIT_DO_NOT_USE                // dwMerit
, 2                               // nPins
, psudSampleSenderPins };        // lpPin
...
// CreateInstance
//
// Provide the way for COM to create a CSampleSender object
//
CUnknown * WINAPI CSampleSender::CreateInstance(LPUNKNOWN punk, HRESULT *phr) 
{
    // assuming we don't want to modify the data
    CSampleSender *pNewObject = new CSampleSender(punk, phr, FALSE);
...
CSampleSender::CSampleSender( IUnknown * pOuter, HRESULT * phr, BOOL ModifiesData )
: CTransInPlaceFilter( TEXT("SampleSender"), (IUnknown*) pOuter, 
                       CLSID_SampleSender, phr, (BOOL)ModifiesData )
, m_callback( NULL )
{
    // this is used to override the input pin with our own
    
    m_pInput = (CTransInPlaceInputPin*) new CSampleSenderInPin( this, phr );
    if( !m_pInput )
    {
        *phr = E_OUTOFMEMORY;
    }
...
STDMETHODIMP CSampleSender::NonDelegatingQueryInterface( REFIID riid, void ** ppv) 
{
    if(riid == IID_ISampleSender) {                
        return GetInterface((ISampleSender *) this, ppv);
    }
    else {
...
HRESULT CSampleSender::CheckInputType( const CMediaType * pmt )
{
...
HRESULT CSampleSender::Receive( IMediaSample * pms )
{
...
HRESULT CSampleSender::Transform( IMediaSample * pms )
{
    CAutoLock lock( &m_Lock );
    ...
    if( m_callback )
    {
        ...
        BOOL * pTypeChanged = &((CSampleSenderInPin*)m_pInput)->m_bMediaTypeChanged;
...
HRESULT CSampleSender::SetCallback( SAMPLECALLBACK Callback )
{
    CAutoLock lock( &m_Lock );
...
STDMETHODIMP CSampleSenderInPin::EnumMediaTypes( IEnumMediaTypes **ppEnum )
{
    ...
    if( !((CSampleSender*)m_pTIPFilter)->OutputPin( )->IsConnected() )
    {
    ...
    return ((CSampleSender*)m_pTIPFilter)->OutputPin( )->GetConnected()->EnumMediaTypes( ppEnum );
...
STDMETHODIMP CSampleSenderInPin::NotifyAllocator( IMemAllocator *pAllocator, BOOL bReadOnly )
{
    if( m_pPrivateAllocator )
    {
        ...
        else
        {
            ...
            if( bReadOnly && !SampleSender( )->IsReadOnly( ) )
            {
                return E_FAIL;
            }
        }
...
STDMETHODIMP CSampleSenderInPin::GetAllocator( IMemAllocator **ppAllocator )
{
    if( m_pPrivateAllocator )
    {
...
HRESULT CSampleSenderInPin::SetDeliveryBuffer( ALLOCATOR_PROPERTIES props, BYTE * pBuffer )
{
    ...
    HRESULT hr = 0;
    m_pPrivateAllocator = new CSampleSenderAllocator( this, &hr );
    if( !m_pPrivateAllocator )
    {
...
HRESULT CSampleSenderInPin::SetMediaType( const CMediaType *pmt )
{
...
HRESULT CSampleSenderAllocator::GetAllocatorRequirements( ALLOCATOR_PROPERTIES *pProps )
{
...
void CSampleSenderAllocator::ReallyFree()
{
    // look at the base class code to see where this came from!
