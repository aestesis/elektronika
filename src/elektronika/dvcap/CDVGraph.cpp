...
HRESULT CDVGraph::InitializeGraph()
{
...

	{
		...
		CHECK_ERROR( TEXT(" Failed to  QI ISampleGrabber basic filter."), hr);

		hr = pF->QueryInterface(IID_ISampleGrabber, reinterpret_cast<void**>(&m_pSampleGrabber));
		CHECK_ERROR( TEXT(" Failed to  QI ISampleGrabber."), hr);
		
		hr = m_pGraph->AddFilter(pF, L"SampleGrabber");
		CHECK_ERROR( TEXT(" Failed to  add ISampleGrabber."), hr);
		...
		CHECK_ERROR( TEXT(" Failed to set ISampleGrabber resolution."), hr);
	}
...
/*
    HRESULT hr = m_pCaptureGraphBuilder->Render
	if(hr!=S_OK)
		return hr;
*/
...
bool CDVGraph::getGrabSize(int *w, int *h)
{
	HRESULT	hr;
	...
	*w=pVideoHeader->bmiHeader.biWidth;
	*h=pVideoHeader->bmiHeader.biHeight;

	return true;
}

bool CDVGraph::getGrabBuffer(void *buffer)
{
	HRESULT	hr;
	long size = 0; 
