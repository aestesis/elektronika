...
AppVerName=Aestesis Elektronika Live 2.71
...
WizardImageFile=\aestesis\code\elektroSDK\elektronika\installer-icon-1.bmp
BackColor=$0080ff
BackColor2=clYellow
WizardSmallImageFile=\aestesis\code\elektroSDK\elektronika\installer-icon-2.bmp
...
; shaders / sources
Source: shaders\sources\colorkey.fx; DestDir: {app}\shaders\sources
Source: shaders\sources\tint.fx; DestDir: {app}\shaders\sources
...
;Source: klcodec345b.exe; Destdir: {tmp}
;Source: qtlite111.exe; Destdir: {tmp}
...
;Source: install_flash_player.exe; DestDir: {tmp}
...
[Run]
;Filename: {tmp}\klcodec345b.exe; WorkingDir: {tmp}
;Filename: {tmp}\qtlite111.exe;  WorkingDir: {tmp}
...
;Filename: {tmp}\install_flash_player.exe; WorkingDir: {tmp}

[Messages]
BeveledLabel=aestesis.eu
...
begin
// MsgBox('Please visit aestesis.eu to download third party components if the installation fail', mbInformation, MB_OK);
// ShellExec('', 'http://aestesis.eu/sub/download.html', '', '', SW_SHOW, ewNoWait, ErrorCode);
  Result := true;
end;
